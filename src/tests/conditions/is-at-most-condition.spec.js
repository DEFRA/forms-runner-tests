...
import { IsAtMostCondition } from '../../conditions/is-at-most-condition.js'
...
test.describe('IsAtMostCondition', () => {
  test('NumberValue: trigger/non-trigger values are correct', () => {
    const condition = new IsAtMostCondition({
      id: 'cond-1',
      name: 'number-at-most',
      operator: 'is at most',
      componentId: 'comp-1',
  ...
  test('RelativeDate (in the future): trigger is on/before boundary; non-trigger is after boundary', () => {
    const relative = RELATIVE_FUTURE_10_DAYS
    const boundary = stripTime(getTriggerDate(relative))

    const condition = new IsAtMostCondition({
      id: 'cond-2',
      name: 'relative-at-most-future',
      operator: 'is at most',
      componentId: 'comp-2',
      type: 'RelativeDate',
      value: relative
  ...
  test('RelativeDate (in the past): trigger is on/after boundary; non-trigger is before boundary', () => {
    const relative = RELATIVE_PAST_10_DAYS
    const boundary = stripTime(getTriggerDate(relative))

    const condition = new IsAtMostCondition({
      id: 'cond-3',
      name: 'relative-at-most-past',
      operator: 'is at most',
      componentId: 'comp-3',
      type: 'RelativeDate',
      value: relative
